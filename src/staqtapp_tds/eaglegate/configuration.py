...
class EaglegateConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode not in {EaglegateMode.TARGET_ONLY, EaglegateMode.SHADOW}:
            raise EaglegateContractError(
                "v3.8 configuration supports target-only or shadow only",
                fault=EaglegateFault.AUTHORITY_REJECTED,
            )
        if self.canary_basis_points != 0:
            raise EaglegateContractError(
                "canary allocation has no authority in v3.8",
                fault=EaglegateFault.AUTHORITY_REJECTED,
            )
