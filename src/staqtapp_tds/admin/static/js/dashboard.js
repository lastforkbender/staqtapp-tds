...
const setElementText = (el, value) => {
  if (!el) return false;
  const next = String(tr(value ?? '—'));
  if (el.textContent === next) return false;
  el.textContent = next;
  return true;
};
const setInlineStyle = (el, property, value) => {
  if (!el || el.style[property] === value) return false;
  el.style[property] = value;
  return true;
};
const setText = (id, value) => setElementText($(id), value);
const setWidth = (id, value) => setInlineStyle($(id), 'width', `${Math.max(0, Math.min(100, Number(value) || 0))}%`);
const renderSignatures = new WeakMap();
const renderIfChanged = (host, signature, build) => {
  if (!host) return false;
  const next = typeof signature === 'string' ? signature : JSON.stringify(signature);
  if (renderSignatures.get(host) === next) return false;
  const fragment = document.createDocumentFragment();
  build(fragment);
  host.replaceChildren(fragment);
  renderSignatures.set(host, next);
  return true;
};
...
const setCompactText = (id, value, fallback = 0) => {
  const el = $(id);
  if (!el) return;
  setElementText(el, compactCount(value, fallback));
  const n = Number(value);
  const title = Number.isFinite(n) ? n.toLocaleString() : String(value ?? fallback);
...
const DASHBOARD_PAGES = Object.freeze([
  'overview', 'architecture', 'performance', 'timeline', 'diagnostics', 'pressure',
  'csv-interpole', 'snapshots', 'locks', 'behavior', 'spiral-rank', 'indexes',
  'storage', 'comparison', 'recovery', 'recommendations', 'alerts', 'security',
  'configuration'
]);
const dashboardPageSet = new Set(DASHBOARD_PAGES);
let latestDashboardData = null;
const pageTokens = (el, attribute) => String(el.getAttribute(attribute) || '').split(/\s+/).filter(Boolean);
const pageFromHash = () => {
  let page = '';
  try { page = decodeURIComponent(window.location.hash.slice(1)); } catch (_) { page = ''; }
  return dashboardPageSet.has(page) ? page : '';
};
const startupPage = () => {
  try {
    const configured = window.TDSBrowserSettings && window.TDSBrowserSettings.load().startupPage;
    return dashboardPageSet.has(configured) ? configured : 'overview';
  } catch (_) {
    return 'overview';
  }
};
...
function activateDashboardPage(page, options = {}) {
  const next = dashboardPageSet.has(page) ? page : 'overview';
  const target = $(next);
  if (!target) return false;
  document.querySelectorAll('[data-dashboard-page]').forEach((el) => {
    setHidden(el, !pageTokens(el, 'data-dashboard-page').includes(next));
  });
  document.querySelectorAll('[data-dashboard-page-shell]').forEach((el) => {
    setHidden(el, !pageTokens(el, 'data-dashboard-page-shell').includes(next));
  });
  document.querySelectorAll('a.nav-pill[href^="#"]').forEach((link) => {
    const active = link.getAttribute('href') === `#${next}`;
  ...
  if (main && main.dataset.activePage !== next) main.dataset.activePage = next;
  if (options.history === 'push' && window.location.hash !== `#${next}`) {
    window.history.pushState({ tdsPage: next }, '', `#${next}`);
  } else if (options.history === 'replace' && window.location.hash !== `#${next}`) {
    window.history.replaceState({ tdsPage: next }, '', `#${next}`);
  }
  if (options.resetScroll) window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
  if (latestDashboardData && options.render !== false) renderActiveDashboardPage(latestDashboardData, next);
  return true;
}
function initNavigation() {
  const links = Array.from(document.querySelectorAll('a.nav-pill[href^="#"]'));
  links.forEach((link) => {
    const page = link.getAttribute('href').slice(1);
    link.setAttribute('aria-controls', page);
    link.addEventListener('click', (event) => {
      if (event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
      event.preventDefault();
      activateDashboardPage(page, { history: 'push', resetScroll: true });
    });
  });
  window.addEventListener('hashchange', () => {
    const requested = pageFromHash();
    activateDashboardPage(requested || 'overview', { history: requested ? 'none' : 'replace', resetScroll: true });
  });
  const direct = pageFromHash();
  activateDashboardPage(direct || startupPage(), { history: direct ? 'none' : 'replace', resetScroll: false });
}
window.TDSDashboardNavigation = { pages: DASHBOARD_PAGES, activate: activateDashboardPage, current: pageFromHash };
function renderNamespaces(behavior){
  const host=$('namespaces'); if(!host)return;
  ...
  const max=Math.max(...visible.map(x=>Number(x.ops)||0),1);
  const changed=renderIfChanged(host, visible, (target)=>{
  ...
  if (changed && window.TDSI18N) window.TDSI18N.applyTranslations(host);
}
function renderRecommendations(data){
  const host=$('recommendations-list'); if(!host)return;
  ...
  const rows=[...list.map(x=>[x.title||x.name||'Recommendation',x.detail||x.message||String(x)]),...synthetic].slice(0,4);
  const changed=renderIfChanged(host, rows, (target)=>{
    rows.forEach(([title,body])=>{
      const row=document.createElement('div'); row.className='reco'; const div=document.createElement('div'); div.appendChild(node('b', title)); div.appendChild(node('span', body)); row.appendChild(div); target.appendChild(row);
    });
  });
  if (changed && window.TDSI18N) window.TDSI18N.applyTranslations(host);
}
function renderTimeline(data){
  const host=$('timeline-list'); if(!host)return;
  ...
  const changed=renderIfChanged(host, {timeline, rows}, (target)=>{
    if(timeline.length){
    ...
    rows.forEach(([t,title,body])=>appendTimelineRow(target,t,title,body));
  });
  if (changed && window.TDSI18N) window.TDSI18N.applyTranslations(host);
}

function renderNativeDiagEvents(nativeDiag){
  const host=$('native-diag-events'); if(!host)return;
  const events=Array.isArray(nativeDiag.recent_events)?nativeDiag.recent_events.slice(-8).reverse():[];
  const changed=renderIfChanged(host, events, (target)=>{
    if(!events.length){appendPairRow(target,'diag-event-row','No native transition events yet','Start native operations or enable diagnostics.'); return;}
    events.forEach(ev=>{
...
function applyComponentStatus(components, storage){
  ...
  Object.entries(map).forEach(([ui,key])=>{
    ...
    setElementText(el, titleCase(v));
    setInlineStyle(el, 'color', String(v).includes('disabled') ? 'var(--orange)' : 'var(--green)');
  });
}
function renderPressureEngine(pressure){
  ...
  const host=$('pressure-causes'); if(!host)return;
  const causes=Array.isArray(pressure.causes)?pressure.causes.slice(0,5):[];
  const changed=renderIfChanged(host, causes, (target)=>{
    if(!causes.length){appendPairRow(target,'diag-event-row','Stable','No elevated pressure detected from current snapshots.'); return;}
    causes.forEach((cause,i)=>appendPairRow(target,'diag-event-row',i===0?'Dominant signal':'Supporting signal',String(cause)));
  });
  if (changed && window.TDSI18N) window.TDSI18N.applyTranslations(host);
}
...
function renderRecoveryPlanner(recovery){
  ...
  const host=$('recovery-actions'); if(host){
    ...
    const changed=renderIfChanged(host, actions, (target)=>{
      if(!actions.length){
        appendRecoveryAction(target,'recovery-action','Observe only','No recovery action is recommended from the current snapshot.','automatic: no','');
      } else {
        actions.forEach(action=>{
          const evidence=Array.isArray(action.evidence)?action.evidence.slice(0,3).join(' '):'';
          appendRecoveryAction(target,`recovery-action ${action.severity||'info'}`,action.title||action.code||'Recovery action',action.recommendation||'',`${titleCase(action.subsystem||'system')} · confidence ${fmt(action.confidence||0,0)}% · automatic: ${action.automatic?'yes':'no'}`,evidence);
        });
      }
    });
    if (changed && window.TDSI18N) window.TDSI18N.applyTranslations(host);
  ...
  const guards=$('recovery-guardrails'); if(guards){
    ...
    const rows=(list.length?list:['Planner consumes copied snapshots only.','No automatic storage mutation is allowed.']).slice(0,4);
    const changed=renderIfChanged(guards, rows, (target)=>{
      rows.forEach(g=>{
        const row=document.createElement('p'); const img=document.createElement('img'); img.src='/static/icons/security.svg'; img.alt=''; row.appendChild(img); row.appendChild(node('span', g)); target.appendChild(row);
      });
    });
    if (changed && window.TDSI18N) window.TDSI18N.applyTranslations(guards);
  }
}
...
function renderSpiralRank(data){
  ...
  const top=$('spiral-rank-top'); if(top){
    ...
    renderIfChanged(top, visible, (target)=>{
      if(!rows.length){ appendPairRow(target,'diag-event-row','No ranked traces yet','Run Spiral Rank and publish the run to admin telemetry.'); }
      visible.forEach(r=>{
        const rank=fmt(r.rank,0), score=fmt(r.score,0), conf=fmt(r.confidence,0), depth=fmt(r.depth,0);
        appendPairRow(target,'diag-event-row',`#${rank} ${r.trace_id||'trace'}`,`score ${score} · confidence ${conf} · depth ${depth} · ${r.native?'native':'python'}`);
      });
    });
  }
  const hist=$('spiral-rank-history'); if(hist){
    const history=Array.isArray(sr.history)?sr.history.slice(-24):[];
    renderIfChanged(hist, history, (target)=>{
      if(!history.length){ target.appendChild(node('p','Waiting for Spiral Rank run history.','panel-note')); }
      else {
        ...
        appendPairRow(target,'diag-event-row','Latest run',`${fmt(latest.elapsed_ms,0)} ms · ${fmt(latest.ranked_count,0)} ranked · ${latest.engine||'engine'}`);
      }
    });
  }
}

function renderCompletedTelemetryPages(data, page){
...
function renderAlertsPage(data){
  ...
  const changed=renderIfChanged(host, visible, (target)=>{
    visible.forEach(([level,title,body])=>{
...
function renderCSVInterpoleMonitor(data){
  ...
  const ring=$('csv-ring-score'); if(ring && ring.parentElement) setInlineStyle(ring.parentElement, 'background', `conic-gradient(var(--blue) 0 ${score}%, rgba(255,255,255,.10) ${score}% 100%)`);

  const cards=$('csv-monitor-cards'); if(cards){
    const items=monitor && Array.isArray(monitor.cards)?monitor.cards:[];
    renderIfChanged(cards, items, (target)=>{
      if(!items.length){
        ...
        row.append(img, div); target.appendChild(row);
      } else {
        items.forEach(card=>{
  ...
  const nodes=$('csv-ring-nodes'); if(nodes){
    ...
    renderIfChanged(nodes, items, (target)=>{
      if(!items.length) appendPairRow(target,'csv-node-row watch','Ring waiting','Committed Interpole ring evidence has not been surfaced to this Browser snapshot.');
      items.forEach(item=>{
        ...
        row.appendChild(small); target.appendChild(row);
      });
    });
  }
  ...
  const gates=$('csv-gate-stack'); if(gates){
    ...
    renderIfChanged(gates, items, (target)=>{
      if(!items.length) appendPairRow(target,'csv-gate-row watch','Gate stack waiting','Performance and readiness gates are not yet present in this Browser snapshot.');
      items.forEach(gate=>{
  ...
  const lanes=$('csv-signal-lanes'); if(lanes){
    ...
    renderIfChanged(lanes, items, (target)=>{
      if(!items.length) appendPairRow(target,'csv-signal-row watch','Signals waiting','Determinant pressure lanes have not been surfaced yet.');
      items.forEach(lane=>{
        ...
        div.appendChild(small); row.append(img,div); target.appendChild(row);
      });
    });
  }
  ...
  const events=$('csv-event-log'); if(events){
    ...
    renderIfChanged(events, items, (target)=>{
      if(!items.length) appendPairRow(target,'csv-event-row watch','Waiting','No CSV Interpole monitor events in the current snapshot.');
      items.forEach(ev=>appendPairRow(target,`csv-event-row ${csvStatusClass(ev.status)}`,`${ev.event_index}. ${titleCase(ev.event_kind)}`,`${titleCase(ev.status)} · ${ev.message || ''}${ev.fingerprint_suffix ? ' · fp '+ev.fingerprint_suffix : ''}`));
    });
  }
}

function renderActiveDashboardPage(data, page) {
  const n=normalize(data);
  switch (page) {
    case 'architecture': applyComponentStatus(n.components, n.storage); break;
    case 'behavior': renderNamespaces(n.behavior); break;
    case 'timeline': renderTimeline(data); break;
    case 'diagnostics': renderNativeDiagEvents(n.nativeDiagnostics); break;
    case 'pressure': renderPressureEngine(n.pressure); break;
    case 'csv-interpole': renderCSVInterpoleMonitor(data); break;
    case 'spiral-rank': renderSpiralRank(data); break;
    case 'snapshots':
    case 'locks':
    case 'comparison': renderCompletedTelemetryPages(data, page); break;
    case 'recovery': renderRecoveryPlanner(n.recovery); break;
    case 'recommendations': renderRecommendations(data); break;
    case 'alerts': renderAlertsPage(data); break;
    default: break;
  }
}

function renderWorkspaceMount(data) {
  const chip=$('workspace-mount-chip');
  if(!chip) return;
  const mount=data && data.workspace_mount;
  if(!mount || typeof mount!=='object'){
    setHidden(chip, true);
    return;
  }
  const raw=String(mount.state || 'unavailable').toLowerCase();
  const state=['ready','stale','unavailable','invalid'].includes(raw) ? raw : 'invalid';
  setHidden(chip, false);
  if(chip.dataset.state!==state) chip.dataset.state=state;
  setText('workspace-mount-state', titleCase(state));
}
...
function render(data){
  try{
    ...
    const donut=$('donut'); if(donut) setInlineStyle(donut, 'background', `conic-gradient(var(--blue) 0 ${readPct}%, var(--orange) ${readPct}% ${readPct+writePct}%, var(--purple) ${readPct+writePct}% ${readPct+writePct+maintenancePct}%, var(--green) ${readPct+writePct+maintenancePct}% 100%)`);
...
async function refreshDashboard(signal){
  ...
  try{
    ...
    if(!document.hidden){
      setText('health-main','DISCONNECTED');
      setText('health-sub','Waiting for local admin server');
      console.error(err);
    }
    return false;
  }
}
window.TDSDashboardRefresh = {
  timer:null,
  controller:null,
  running:false,
  generation:0,
  ...
  refreshMS(){
    return window.TDSBrowserSettings ? window.TDSBrowserSettings.getRefreshMS() : (window.STAQTAPP_REFRESH_MS || 2000);
  },
  clearTimer(){
    if(this.timer!==null){ clearTimeout(this.timer); this.timer=null; }
  },
  schedule(delay, forceOnce=false){
    this.clearTimer();
    if(document.hidden) return;
    const generation=this.generation;
    this.timer=setTimeout(()=>{
      this.timer=null;
      this.run(generation, forceOnce);
    }, Math.max(0, Number(delay)||0));
  },
  async run(generation=this.generation, forceOnce=false){
    if(generation!==this.generation || document.hidden) return;
    if(!forceOnce && this.refreshMS()<=0) return;
    if(this.running){ this.pendingImmediate=true; return; }
    this.running=true;
    this.controller=new AbortController();
    try{
      const fetched=await refreshDashboard(this.controller.signal);
      this.hasFetched=this.hasFetched||fetched;
    }finally{
      this.running=false;
      ...
      if(this.pendingImmediate){
        ...
        this.schedule(0, !this.hasFetched);
      }else if(generation===this.generation){
        const delay=this.refreshMS();
  ...
  restart(immediate=false){
    ...
    const delay=this.refreshMS();
    if(!document.hidden && delay>0) this.schedule(immediate ? 0 : delay);
  },
  pause(){
    this.generation+=1;
    this.pendingImmediate=false;
  ...
  resume(){
    if(document.hidden) return;
    const shouldFetch=this.refreshMS()>0 || !this.hasFetched;
...
document.addEventListener('visibilitychange',()=>{
  if(document.hidden) window.TDSDashboardRefresh.pause();
  else window.TDSDashboardRefresh.resume();
});
initNavigation();
window.TDSDashboardRefresh.schedule(0, true);
