...
This is a contract-first, non-activating Eaglegate slice stacked on the v3.7
atomic CSV Generation Plane reference branch. It must not merge ahead of the
Foundation Repair and atomic generation work beneath it.

The installed package version is intentionally unchanged. This slice does not
publish a release, activate speculative decoding, or claim GPU acceleration.
...
Canary and active epochs cannot be constructed without a qualification root.
A later exactness laboratory will produce the authoritative qualification
artifact; this foundation only establishes its immutable contract.

### Content-free evidence

`staqtapp_tds.eaglegate.evidence` provides:

- bounded episode receipts containing counts, timings, resource accounting,
  fallback reasons, and fault classes; and
- append-only epoch-state receipts with predecessor-root validation.
...
```text
DRAFT -> QUALIFIED -> STAGED -> SHADOW -> CANARY -> ACTIVE
```

Retirement and quarantine are terminal. Canary may return to shadow. Invalid
jumps, cross-epoch chains, and qualification replacement inside one epoch are
rejected.
...
atomic replacement, and best-effort directory `fsync`.
...
All commands emit deterministic JSON. `evaluate` is simulation-only. The console
has no activation endpoint and reports `activation_authority: false`.
...
- stage, canary, promote, or rollback executors;
...
The next Eaglegate slice should be the differential exactness laboratory and a
mock target/proposer adapter boundary. It should prove greedy token identity,
lossless sampled-distribution preservation, committed-KV equivalence,
cancellation cleanup, epoch pinning, telemetry overflow containment, and
unconditional target-only fallback before any real EAGLE runtime is admitted.
