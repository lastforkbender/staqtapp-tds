...
bytes, verifies the content-free boundary, and publishes observer evidence. An
...
The next tranche must define a runtime-adapter conformance ABI with exact
identity, reserve/propose/verify/commit/rewind/cancel/release sequencing,
exception containment, deadline behavior, RNG ownership, logits-processor
ordering, termination behavior, and epoch lifetime. A real EAGLE runtime remains
unadmitted until that ABI and its adversarial fake-runtime suite pass.
