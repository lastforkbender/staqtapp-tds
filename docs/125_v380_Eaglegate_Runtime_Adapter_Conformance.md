...
The next step should be a shadow-only adapter SDK and one explicitly named,
non-executing integration fixture. It must translate a real runtime's capability
metadata into the immutable identity without invoking a model, then prove that
unsupported sampler, RNG, logits-processor, termination, KV, kernel, or deadline
contracts remain target-only. Actual model execution remains unadmitted until a
specific runtime adapter, hardware lane, and lossless differential corpus are
qualified together.
