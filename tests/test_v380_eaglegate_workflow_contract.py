...
def test_focused_ci_requires_cross_platform_lossless_contract_lanes():
    ...
    assert "agent/v380-phase1-phase4-convergence" in workflow
    assert "ubuntu-24.04" in workflow
    assert "macos-14" in workflow
    assert "windows-2022" in workflow
    assert "tests/test_v380_eaglegate_lossless_foundation.py" in workflow
    assert "tests/test_v380_eaglegate_generation_authority.py" in workflow
    ...
    assert "agent/v370-atomic-csv-reference" not in workflow
