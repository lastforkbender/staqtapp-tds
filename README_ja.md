...
## v2.9.4 非停止 Result コントラクト

v2.9.4 では、AI システムと長時間稼働サービス向けに、公開 TDS API の返却契約を明確化しました。通常運用で失敗し得る公開 TDS 操作は、TDS 由来の例外で呼び出し側の実行を停止させたり、曖昧な boolean 失敗値を返したりせず、標準化された `TDSResult` を返します。

呼び出し側は 1 つの予測可能なパターンで処理できます。
...
`TDSResult` は、制御済み TDS 結果のための単一の公開 success/failure エンベロープです。主なフィールドは次の通りです。

- `ok`: 成功なら `True`、制御済み失敗なら `False`。
- `code`: 安定した機械可読 result code。
- `message`: 人間向け説明。
- `name` / `path`: 任意の TDS 位置情報。
- `value`: 必要に応じて返されるオブジェクトまたは操作ペイロード。
- `meta`: ログ、テレメトリ、再試行方針、AI 判断用の構造化診断情報。

正式な result-code のソースは次のファイルです。

```text
src/staqtapp_tds/result.py
```

すべての公開 `TDSResult.code` は `TDSResultCode` に定義され、`TDS_RESULT_REGISTRY` に説明があります。その registry から次の人間向け・機械向けリファレンスが生成されます。
...
通常の TDS 制御下の失敗に対する設計保証:

- 公開 result-first 操作は `TDSResult` を返します。
- 通常の保存、decode、missing entry、validation、捕捉済み環境エラーで、TDS は意図的に呼び出し側の実行を停止させません。
- Result code は runtime registry に集中管理され、独立した status 文字列としてコードベースに散在しません。
- Result-code ドキュメントは runtime registry から生成され、ドキュメントの drift を防ぎます。
- raw value を返す互換メソッドは `read_value()`、`write_entry()`、`delete_entry()` のように明示的な名前で残されます。

この保証は、Python インタプリタ終了、OS 障害、致命的な native crash、`KeyboardInterrupt`、回復不能なメモリ不足など、TDS の制御外のプロセスレベル障害を防ぐという意味ではありません。

## v2.9.4 Result Registry Discipline

Staqtapp-TDS は、すべての公開 `TDSResult.code` を 1 つの runtime source of truth、`src/staqtapp_tds/result.py` に定義します。コード比較には `TDSResultCode`、機械可読 metadata には `result_info(code)` を使用します。Markdown と JSON の result-code リファレンスは、この registry から生成されます。

## ハイライト

- セマンティックルーティングゾーンと予約名前空間ポリシーを備えたディレクトリ中心 VFS API。
- 公開 result-first 操作用の標準化された非停止 `TDSResult` return envelope。
- Markdown / JSON リファレンスを生成する中央 `TDSResultCode` registry。
