...
|---|---|---|
| Binary directory header | ✅ Complete | 36-byte struct, CRC32 verified |
| Numba JIT math kernels | ✅ Complete | 6 kernels, cached compilation |
| Probability-LRU registry | ✅ Complete | Decay-weighted, thread-safe |
| Loop cache manager | ✅ Complete | Cycle slots, numpy batch flush |
| ConcurrencyPool singleton | ✅ Complete | Thread + process + async |
| Symbol table + matrix swap | ✅ Complete | Numba in-place kernel |
| Recursive array join | ✅ Complete | Subtree numpy concat |
| mmap-backed reader | ✅ Complete | O(log n) seek, lazy load |
| Atomic shadow-swap writer | ✅ Complete | fsync + rename |
| Parallel flush | ✅ Complete | ParallelFlusher, concurrent nodes |
| Encryption layer | 🔲 Planned | AES-256-GCM per entry, key in header |
| Distributed mount | 🔲 Planned | Shard `.tds` files across nodes |
| Streaming write mode | 🔲 Planned | Append without full rewrite |
| Typed tensor extension | 🔲 Planned | `FmtID.TENSOR_4D` with shape metadata |
| WAL (write-ahead log) | 🔲 Planned | Recovery from interrupted flush |
| Watch / notify | 🔲 Planned | Callback on entry mutation |

---

*TDS v0.1.0 — Temporal Directory System — built for the age of ASI.*
